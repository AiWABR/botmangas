...
def _plan_text(user_id: int, access: dict | None) -> str:
    ...
    if not access:
        return (
            f"📥 <b>Meu plano offline</b>\n\n"
            f"» <b>Status:</b> <i>{status}</i>\n"
            f"» <b>ID:</b> <code>{user_id}</code>\n\n"
            f"Você ainda não tem um plano ativo no <b>{brand}</b>.\n"
            "Escolha um plano abaixo para liberar os PDFs offline."
        )

    plan = access.get("plan") or ""
    expires_at = access.get("expires_at") or ""
    expires_text = "não expira" if access.get("is_lifetime") else _format_local_datetime(expires_at)
    remaining = "não expira" if access.get("is_lifetime") else _remaining_label(expires_at)

    return (
        f"📥 <b>Meu plano offline</b>\n\n"
        f"» <b>Status:</b> <i>{html.escape(status)}</i>\n"
        f"» <b>Plano:</b> <i>{html.escape(plan_label(plan))}</i>\n"
        f"» <b>Duração:</b> <i>{html.escape(_duration_label(plan))}</i>\n"
        f"» <b>Válido até:</b> <i>{html.escape(expires_text)}</i>\n"
        f"» <b>Tempo restante:</b> <i>{html.escape(remaining)}</i>\n"
        f"» <b>ID:</b> <code>{user_id}</code>\n\n"
        "Use o botão abaixo para renovar ou falar com o suporte."
    )

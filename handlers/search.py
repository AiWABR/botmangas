...
def _build_search_text(user_id: int | None, query: str, page: int, total: int) -> str:
...
def render_search_page(context: ContextTypes.DEFAULT_TYPE, token: str, page: int, user_id: int | None = None) -> dict | None:
    ...
    return {
        "text": _build_search_text(user_id, query, page, total),
...
async def buscar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        await message.reply_text(t_user(user.id, "common.private_only"), parse_mode="HTML")
        return
    ...
    if not query:
        await message.reply_text(t_user(user.id, "search.how_to"), parse_mode="HTML")
        return
    if len(query) < 2:
        await message.reply_text(t_user(user.id, "search.too_short"), parse_mode="HTML")
        return
    if _is_search_cooldown(context, user.id, query):
        await message.reply_text(t_user(user.id, "search.cooldown"), parse_mode="HTML")
        return
    if _is_inflight(user.id, query):
        await message.reply_text(t_user(user.id, "search.inflight"), parse_mode="HTML")
        return
    ...
    async with lock:
        ...
        try:
            if cached_results is not None:
                ...
                loading = await message.reply_text(t_user(user.id, "search.loading"), parse_mode="HTML")
            ...
            if not results:
                ...
                edited = await _safe_edit_loading(loading, t_user(user.id, "search.no_results"))
                if not edited:
                    await message.reply_text(t_user(user.id, "search.no_results"), parse_mode="HTML")
                return

            token = store_search_session(context, query, results)
            rendered = render_search_page(context, token, 1, user.id)
            if not rendered:
                edited = await _safe_edit_loading(loading, t_user(user.id, "search.error"))
                if not edited:
                    await message.reply_text(t_user(user.id, "search.error"), parse_mode="HTML")
                return
            ...
            edited = await _safe_edit_loading(loading, t_user(user.id, "search.timeout"))
            if not edited:
                await message.reply_text(t_user(user.id, "search.timeout"), parse_mode="HTML")
...
async def buscar_texto_livre(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
